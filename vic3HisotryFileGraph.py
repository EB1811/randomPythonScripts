...
def extractCountriesInfo(
    countryTags, wantedInfo, divisionInfo=None, path=corePath + "/swiss.history"
):
    with open(path, "r") as file:
        ...
        for date, dateGameData in dates.items():
            if date.endswith(".1.1"):
                ...
                requestedCountries = list(
                    filter(lambda c: c["tag"] in countryTags, dateGameData["countries"])
                )

                if len(requestedCountries) != len(countryTags):
                    for missingTag in filter(
                        lambda t: t
                        not in map(lambda c: c["tag"], dateGameData["countries"]),
                        countryTags,
                    ):
                ...
                for country in requestedCountries:
                    wantedCalcData = (
                        country[wantedInfo] / country[divisionInfo]
                        if divisionInfo
                        else country[wantedInfo]
                    )
                    formattedCountries.append(
                        {"tag": country["tag"], "data": wantedCalcData}
                    )
        ...
        countryData.append(
            {
                ...
                "dataPoints": list(
                    map(
                        lambda cs: next(c for c in cs if c["tag"] == tag)["data"],
                        dateCountries,
                    )
                ),
            }
...
createHistoryGraph(
    ["FRA", "USA", "TUR", "ENG", "HND", "RUS", "AUS", "PRU", "NGF", "KUK", "SWI"], "gdp"
)
