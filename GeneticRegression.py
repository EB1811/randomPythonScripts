...
def evalSymbReg(individual):
    ...
    if UseSqError:
        # squared error
        error = (
            abs(func(x) - measurement(idx, x)) ** 2 for idx, x in enumerate(test_points)
        )
    else:
        # Absolute distance between target curve and solution
        error = (
            abs(func(x) - measurement(idx, x)) for idx, x in enumerate(test_points)
        )
...
toolbox.decorate(
    "mate", gp.staticLimit(key=operator.attrgetter("height"), max_value=64)
)
toolbox.decorate(
    "mutate", gp.staticLimit(key=operator.attrgetter("height"), max_value=64)
)
...
pop, log = algorithms.eaSimple(
    pop, toolbox, p_xo, p_m, no_generations, stats=mstats, halloffame=hof, verbose=True
)
